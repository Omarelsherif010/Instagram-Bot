...
for user in username_list:
    user_id_code = client.user_id_from_username(user)
    users_id_list.append(user_id_code)

def get_and_post_messages():
...
### Main Timer to repeat the whole process
Main_timer_count_in_seconds = (len(users_id_list) * 30 * 60) + (30 * 60)
schedule.every(Main_timer_count_in_seconds).seconds.do(get_and_post_messages)
